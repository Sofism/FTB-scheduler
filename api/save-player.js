...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!name || typeof name !== 'string') return res.status(400).json({ error: 'Missi
    if (playerData === undefined || playerData === null) return res.status(400).json({
    ...
    if (name.startsWith('__')) return res.status(200).json({ ok: true, name });
    const webhookUrl = process.env.DISCORD_WEBHOOK_URL;
    if (webhookUrl && typeof playerData === 'object') {
      try {
        ...
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({
            embeds: [{
              ...
              title: (isNew ? '[NEW] ' : '[UPDATE] ') + name + ' ' + action,
              fields: [
